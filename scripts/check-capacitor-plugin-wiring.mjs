 ...
 * - CocoaPods podspec exists
 * - SwiftPM Package(name: ...) and the single .library(name: ...) match the product name
 *   generated by Capacitor from the npm package name
 *
 * Usage:
...
function capacitorGeneratedSpmProductName(packageName) {
...
if (supportsIos) {
  ...
  if (!exists(pkgSwift)) {
    ...
    const generatedProductName = capacitorGeneratedSpmProductName(pkg.name);
    if (generatedProductName && pkgName !== generatedProductName) {
      errors.push(
        `SPM: Package(name)=${pkgName} != Capacitor generated package/product name ${generatedProductName} from npm package ${pkg.name}`
      );
    }
    if (generatedProductName && !libNames.includes(generatedProductName)) {
      errors.push(
        `SPM: missing Capacitor generated .library(name)=${generatedProductName} from npm package ${pkg.name}; libraries ${JSON.stringify(libNames)}`
      );
    }
    if (libNames.length !== 1) {
      errors.push(`SPM: expected exactly one .library product, got ${JSON.stringify(libNames)}`);
    }
  }
}
